...
# ==========================================================================================
    ...
    with col1:
        # --- อัปเดต: เพิ่มตัวเลือก "พิมพ์กำหนดเอง" ---
        subject_options = ["Auto (สุ่ม)"] + available_subjects + ["✏️ พิมพ์กำหนดเอง (Custom Input)"]
        subject = st.selectbox("1. SUBJECT (วัตถุเป้าหมาย)", subject_options, key="iso_sub")
        
        # --- อัปเดต: แสดงช่องพิมพ์ข้อความถ้าเลือก "พิมพ์กำหนดเอง" ---
        custom_subject = ""
    ...
    if st.button("🚀 Generate ISOLATED Prompts", use_container_width=True, key="iso_btn"):
        ...
        for i in range(prompt_count_iso):
            ...
            # --- อัปเดต: ลอจิกดึงค่า Subject สำหรับแบบพิมพ์เอง ---
            if subject == "✏️ พิมพ์กำหนดเอง (Custom Input)":
