...
if app_mode == "🏛️ เครื่องจักรสร้างแท่นวาง (Podiums)":
    ...
    with st.sidebar:
        ...
        if mj_version in ["8.1", "8.0"]:
            st.warning(f"⚠️ V{mj_version} ยังไม่รองรับระบบ Negative Prompt ระบบจะละเว้นคำสั่งกันเงาให้อัตโนมัติชั่วคราวนะครับ")
    ...
    if st.button("🚀 Generate PERFECT Prompts", use_container_width=True):
        ...
        for i in range(prompt_count):
            ...
            # --- อัปเดต: ถ้าไม่ใช่ V8.0 และ V8.1 ถึงจะให้ใส่ --no ได้ ---
...
# ==========================================================================================
    ...
    if st.button("🚀 Generate ISOLATED Prompts", use_container_width=True, key="iso_btn"):
        ...
        for i in range(prompt_count_iso):
            ...
            final_prompt_iso += f" --ar {aspect_ratio_iso} --s {stylize_value_iso} --style raw --v {mj_version_iso}"
