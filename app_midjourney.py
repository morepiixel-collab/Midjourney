...
if app_mode == "🏛️ เครื่องจักรสร้างแท่นวาง (Podiums)":
    ...
    with st.sidebar:
        ...
        # --- อัปเดต: เพิ่มช่องให้เลือก Midjourney Version ---
    ...
    if st.button("🚀 Generate PERFECT Prompts", use_container_width=True):
        ...
        for i in range(prompt_count):
            ...
            # --- อัปเดต: เปลี่ยน --v ให้ดึงจากตัวแปร mj_version ที่ผู้ใช้เลือก ---
            final_prompt = f"/imagine prompt: {clean_base} --ar {aspect_ratio} --s {stylize_value} --style raw --v {mj_version}"
            
            # --- อัปเดต: ระบบ Auto-Convert แก้อาการ V8.1 ไม่รองรับ --no ---
            if negative_prompt:
                if mj_version == "8.1":
                    # ใช้ระบบ Weight ติดลบแทน --no (ให้ผลลัพธ์เหมือนกันแต่ผ่านฉลุยใน V8.1)
                    final_prompt += f" :: {negative_prompt.strip()}::-0.5"
                else:
...
# ==========================================================================================
    ...
    if st.button("🚀 Generate ISOLATED Prompts", use_container_width=True, key="iso_btn"):
        ...
        for i in range(prompt_count_iso):
            ...
            # --- อัปเดต: เปลี่ยน --v ให้ดึงจากตัวแปร mj_version_iso ที่ผู้ใช้เลือก ---
            final_prompt_iso = f"/imagine prompt: {clean_base_iso} --ar {aspect_ratio_iso} --s {stylize_value_iso} --style raw --v {mj_version_iso}"
            
            # --- อัปเดต: ระบบ Auto-Convert แก้อาการ V8.1 ไม่รองรับ --no ---
            if negative_prompt_iso:
                if mj_version_iso == "8.1":
                    final_prompt_iso += f" :: {negative_prompt_iso.strip()}::-0.5"
                else:
