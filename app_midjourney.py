...
if app_mode == "🏛️ เครื่องจักรสร้างแท่นวาง (Podiums)":
    ...
    with st.sidebar:
        ...
        if mj_version in ["8.1", "8.0"]:
            st.warning(f"⚠️ V{mj_version} ไม่รองรับ --no ระบบจะใช้การบีบบังคับคำศัพท์ (Positive Enforcement) แทน")
        ...
        st.markdown("---")
        st.subheader("🛠️ Layout Override (ปรับโครงสร้างพิเศษ)")
        # --- ปุ่มใหม่: ปิดการสร้างแท่น (No Podium) ---
        is_no_podium = st.checkbox("🚫 ไม่เอาแท่นวาง (No Podium / Flat Surface)", value=False)
        if is_no_podium:
            st.info("ระบบจะปรับโครงสร้างให้เป็นพื้นผิวเรียบๆ (Flat Surface) และนำคำว่า แท่น/ฐาน ออกจาก Prompt ทั้งหมด")
    ...
    with col1:
        scene = st.selectbox("1. SCENE (แท่นวาง/ฉาก)", ["Auto (สุ่มตาม DNA)"] + available_scenes)
    ...
    if st.button("🚀 Generate PERFECT Podiums", use_container_width=True):
        ...
        for i in range(prompt_count):
            ...
            # --- อัปเดต: ลอจิกเมื่อเปิดโหมด No Podium ---
            if is_no_podium:
                # 1. ล้างคำนามที่สื่อถึงแท่นทั้งหมด เปลี่ยนเป็น flat surface
                pattern = r'\b(display plinth|presentation plinth|exhibition plinth|exhibition block|presentation block|display block|geometric block|solid geometric block|solid block|wood block|wooden block|acrylic display blocks|acrylic blocks|pedestal|plinth|platform|cylinder|steps|stand)\b'
                sel_scene = re.sub(pattern, 'flat surface', sel_scene, flags=re.IGNORECASE)
                
                # 2. ปรับ Base Core ให้แบนราบที่สุด
                base_core = "blank empty commercial background, extreme high-end commercial asset, photography award winner, completely clear uninterrupted flat ground surface, strictly no podiums, no pedestals, devoid of any objects or decor"
            else:
            ...
            # --- อัปเดต: เพิ่ม negative prompt ป้องกันแท่น (เฉพาะ V7) ---
            if negative_prompt and mj_version not in ["8.1", "8.0"]:
                current_neg = negative_prompt.strip()
                if is_no_podium:
                    current_neg += ", podium, pedestal, stand, platform, plinth"
                final_prompt += f" --no {current_neg}"
                
            final_prompt += f" --ar {sel_ar} --s {stylize_value} --style raw --v {mj_version}"
            prompts.append(final_prompt)
            
        st.session_state['prompts'] = prompts
        st.success(f"✅ เจนเรียบร้อย {prompt_count} Prompts (โหมด No Podium: {'เปิดใช้งาน' if is_no_podium else 'ปิด'})")
    ...
    if 'prompts' in st.session_state:
        ...
        st.download_button(label="💾 ดาวน์โหลดไฟล์ .txt", data=prompt_text, file_name=f"mockup_v{mj_version}.txt", mime="text/plain", use_container_width=True)
