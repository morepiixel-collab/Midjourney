...
# ==========================================================================================
# 🟩 ส่วนที่ 1: โค้ดสำหรับ "เครื่องจักรสร้างแท่นวาง" 
# ==========================================================================================
if app_mode == "🏛️ เครื่องจักรสร้างแท่นวาง (Podiums)":
    ...
    with st.sidebar:
        ...
        mj_version = st.selectbox("Midjourney Version (--v)", ["8.1", "8.0", "7", "6.1", "6.0"], index=0, key="p_ver")
    ...
    if st.button("🚀 Generate PERFECT Prompts", use_container_width=True):
        ...
        for i in range(prompt_count):
            ...
            # --- อัปเดต: ปรับลำดับไวยากรณ์ เอา :: มาไว้ก่อน -- ---
            if negative_prompt:
                if mj_version == "8.1":
                    final_prompt = f"{clean_base} :: {negative_prompt.strip()}::-0.5 --ar {aspect_ratio} --s {stylize_value} --style raw --v {mj_version}"
                else:
                    final_prompt = f"{clean_base} --no {negative_prompt.strip()} --ar {aspect_ratio} --s {stylize_value} --style raw --v {mj_version}"
            else:
                final_prompt = f"{clean_base} --ar {aspect_ratio} --s {stylize_value} --style raw --v {mj_version}"
                    
            prompts.append(final_prompt)
            
        st.session_state['prompts'] = prompts
        st.success(f"✅ เจนเรียบร้อย {prompt_count} Prompts (หมวด: {selected_industry} | เวอร์ชัน: v{mj_version})")
...
# ==========================================================================================
# ⚪ ส่วนที่ 2: โค้ดสำหรับ "เครื่องจักรสร้างฉากขาว (ISOLATED)" 
# ==========================================================================================
    ...
    if st.button("🚀 Generate ISOLATED Prompts", use_container_width=True, key="iso_btn"):
        ...
        for i in range(prompt_count_iso):
            ...
            if negative_prompt_iso:
                if mj_version_iso == "8.1":
                    final_prompt_iso = f"{clean_base_iso} :: {negative_prompt_iso.strip()}::-0.5 --ar {aspect_ratio_iso} --s {stylize_value_iso} --style raw --v {mj_version_iso}"
                else:
                    final_prompt_iso = f"{clean_base_iso} --no {negative_prompt_iso.strip()} --ar {aspect_ratio_iso} --s {stylize_value_iso} --style raw --v {mj_version_iso}"
            else:
                final_prompt_iso = f"{clean_base_iso} --ar {aspect_ratio_iso} --s {stylize_value_iso} --style raw --v {mj_version_iso}"
                    
            prompts_iso.append(final_prompt_iso)
            
        st.session_state['prompts_isolated'] = prompts_iso
        st.success(f"✅ เจนเรียบร้อย {prompt_count_iso} Prompts (หมวด: {selected_industry_iso} | เวอร์ชัน: v{mj_version_iso})")
